...
if __name__ == '__main__':
    ...
    fitter.setDaughter(0,  1, np.array([ 1.0, -0.5, 0.0]), np.array([-1.0, 0.0, 0.0]), np.diag([0.01] * 6))
    fitter.setDaughter(1, -1, np.array([-1.0, -0.5, 0.0]), np.array([ 1.0, 0.0, 0.0]), np.diag([0.01] * 6))
    fitter.fit(3)
    ...
    print("Variance", fitter.getVariance())
